...
## 2026-04-09
- Fix render sorts de forme (4 bugs) :
  - "Ignorez le malus aux tests lié à votre armure…" restauré : texte résiduel après pattern CA capturé en note
  - Souffle de dragon + Résistance de tradition (FORME DE DRAGON) restaurés : saut bullets gras restreint aux vraies formes (nom gras + marqueur vitesse)
  - Colonne Dégâts vide (toile araignée) : parse effets textuels sans dés, XSLT conditionnel sur `amount`
  - Compétence figée "Athlétisme" → dynamique : `skillName` extrait dans le mapper, émis en XML, rendu dans le XSLT
- Golden AVATAR mis à jour : note "Ignorez le malus…" ajoutée
- `schema/spell.xsd` : `<skillName>` ajouté dans `globalStats`
- Previews batch 03 et 04 régénérés
