...
def parse_languages(lang_match):
    # Séparer les langues standards du spécial (après le point-virgule)
    langs = re.split(r',|;', lang_match.group(1))
    languages = [l.strip().lower() for l in langs if l.strip()]
    special_part = lang_match.group(2).strip() if lang_match.group(2) else None
   
    # Nettoyage des langues classiques
    #languages = [l.strip().lower() for l in standard_part.replace('Langues', '').split(',')]
    
    res = {'standard': languages, 'special_spell': None, 'special_text': None}
    
    if special_part:
        # Détecter si c'est entouré d'étoiles (Markdown pour italique)
        spell_match = re.search(r'\*(.*?)\*', special_part)
        if spell_match:
            res['special_spell'] = spell_match.group(1).strip()
        else:
            res['special_text'] = special_part.strip()
...
def parse_monster_md(content):
    ...
    if lang_match:
        lang_data = parse_languages(lang_match)

        """  langs = re.split(r',|;', lang_match.group(1))
        monster_data['languages'] = [l.strip() for l in langs if l.strip()]
        monster_data['lang_special'] = lang_match.group(2).strip() if lang_match.group(2) else None """
        monster_data['languages'] = lang_data

        # Ampute main_section
        main_section = main_section[lang_match.end():]
        
        print(f"[PARSING]    ✓ Langues trouvées: {monster_data['languages']} - {time.time() - step_start:.3f}s")
    else:
        monster_data['languages'] = []
        print("[PARSING]    ✗ Aucune correspondance trouvée pour les langues")
    ...
    saves = re.search(r'Réf\s+([\+\-]\d+),\s*Vig\s+([\+\-]\d+),\s*Vol([\+\-]\d+)', main_section)
    if saves:
    ...
    strikes = []
    strike_matches = list(re.finditer(r'Corps à corps\s+(.*?)\s+(\+\d+)\s+\((.*?)\),\s+Dégâts\s+(.*?)(?=\n|Corps à corps|Sorts|Vitesse|Capacités|$)', main_section, re.DOTALL))
    strike_count = 0
    ...
    for m in strike_matches:
        ...
        for p in parts:
            d_match = re.search(r'(\d+d\d+[\+\-]?\d*)\s+(.*)', p)
            if d_match:
...
def generate_monster_xml(data, output_path):
    ...
    # Défenses
    step_start = time.time()
    print("[XML] 4. Ajout des défenses (CA, PV, Saves)...")
    ...
    # Perception
    step_start = time.time()
    print("[XML] 5. Ajout de la perception et des sens...")
    ...
    lang_count = 0
    for l in lang_data['standard']:
        etree.SubElement(langs_node, "language").text = l
        lang_count += 1

    if lang_data['special_spell']:
        spec_node = etree.SubElement(langs_node, "special")
        spell_node = etree.SubElement(spec_node, "spell")
        spell_node.text = lang_data['special_spell']
    elif lang_data['special_text']:
        etree.SubElement(langs_node, "specialLang").text = lang_data['special_text']
    # lang = etree.SubElement(monster, "languages")
    # if 'lang_special' in data: 
    #     lang.set("langSpecial", data['lang_special'])
    # languages = etree.SubElement(lang, "languages")
    # lang_count = 0
    # for l in data.get('languages', []):
    #     lang_count += 1
    #     language = etree.SubElement(languages, "language").text = l
    print(f"[XML]    ✓ {lang_count} langues ajoutées - {time.time() - step_start:.3f}s")
    ...
    for strike in data.get('strikes', []):
        ...
        for dmg in strike['damages']:
            ...
            etree.SubElement(dmg_elem, "type").text = dmg['type']
...
output_file = "./output/subset_4/young_empyreal_dragon.xml"
