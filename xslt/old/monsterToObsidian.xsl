...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="/monsters/monster">
    ...
    <xsl:text>, **Dex** </xsl:text><xsl:value-of select="abilities/dex"/>
    <xsl:text>, **Con** </xsl:text><xsl:value-of select="abilities/con"/>
    <xsl:text>, **Int** </xsl:text><xsl:value-of select="abilities/int"/>
    <xsl:text>, **Sag** </xsl:text><xsl:value-of select="abilities/wis"/>
    <xsl:text>, **Cha** </xsl:text><xsl:value-of select="abilities/cha"/>
    <xsl:text>&#10;----&#10;</xsl:text>

    <!-- Défenses (CA, Sauvegardes) -->
    <xsl:text>**CA** </xsl:text><xsl:value-of select="armorClass/value"/>
    <xsl:text> ; **Réf** </xsl:text><xsl:value-of select="saves/ref"/>
    <xsl:text>, **Vig** </xsl:text><xsl:value-of select="saves/fort"/>
    <xsl:text>, **Vol** </xsl:text><xsl:value-of select="saves/will"/>
    <xsl:if test="savingThrowsSpecials/special">
      <xsl:text> ; </xsl:text>
      <xsl:for-each select="savingThrowsSpecials/special">
        <xsl:value-of select="."/>
        <xsl:if test="position() != last()">, </xsl:if>
      </xsl:for-each>
    </xsl:if>
    ...
    <xsl:text>**PV** </xsl:text><xsl:value-of select="health/value"/>
  ...
  <xsl:template name="format-special-block">
    ...
    <xsl:if test="effect">
      <xsl:text>**Effet.** </xsl:text><xsl:value-of select="effect"/><xsl:text>. </xsl:text>
    </xsl:if>
